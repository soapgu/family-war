...
test('房间与角色同步：双人进房、角色选择占用切换放弃、模式/难度切换', { tag: '@stable' }, async ({ dualPlayers, baseURL }) => {
  ...
  // ── 2. 在线人数同步 ─────────────────────────────────────────
  await expect(a.page.getByTestId('room-online-count')).toContainText('2')
  await expect(b.page.getByTestId('room-online-count')).toContainText('2')

  // ── 3. 昵称在对方可见（玩家列表） ───────────────────────────
  await expect(b.page.getByText(a.nickname).first()).toBeVisible()
  await expect(a.page.getByText(b.nickname).first()).toBeVisible()

  // ── 4. A 选择 爸爸 → B 看到 爸爸 被 A 占用 ──────────────────
  await roomA.selectRole('爸爸')
  await roomA.waitForRoleSelected()
  await expect(
    b.page.getByTestId('role-card-status-爸爸')
  ...
  await expect(
    a.page.getByTestId('role-card-status-妈妈')
  ).toContainText(b.nickname, { timeout: 10000 })
  ...
  await expect(
    b.page.getByTestId('role-card-status-爸爸')
  ).toContainText('空闲', { timeout: 10000 })
  // 儿子 卡片出现 A 的昵称
  await expect(
  ...
  // ── 10. A 切换难度为 HARD → B 看到 HARD 按钮高亮（选中态同步） ──
  await roomA.switchDifficulty('hard')
  await expect(
    b.page.getByTestId('room-difficulty-hard')
