...
describe('RoleCard', () => {
  it('shows role name', () => {
    render(<RoleCard role="爸爸" occupant={null} isMine={false} onClick={vi.fn()} />)
  ...
  it('shows occupant nickname when taken', () => {
    render(<RoleCard role="妈妈" occupant={{ nickname: '小红' }} isMine={false} onClick={vi.fn()} />)
    expect(screen.getByText('小红')).toBeInTheDocument()
  })

  it('shows 我 when isMine', () => {
    render(<RoleCard role="爸爸" occupant={{ nickname: '小明' }} isMine={true} onClick={vi.fn()} />)
  ...
  it('calls onClick with role when free and clicked', async () => {
    ...
    await userEvent.click(screen.getByText('爸爸'))
