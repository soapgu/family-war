...
# v3.6：游戏链路稳定性与客户端回归保障实施计划

> Phase 1“游戏端 E2E 基线”已经完成；当前继续细化 Phase 2“服务端房间与对局生命周期治理”。全量回归和版本发布将在后续阶段补充。

## 1. 当前基础

Phase 1 已使用 Playwright 和 Chromium 建立经过连续执行验证的真实浏览器 E2E。测试访问本地 Vite 页面并通过开发代理连接真实 Koa、Socket.IO 服务，当前基础包括：

- 11 个带 `@stable` Tag 的独立场景，覆盖房间、角色、三种游戏、认输、返回房间、重赛和主动退出；
- 3 个带 `@lifecycle-issue` Tag 的生命周期场景，其中断线重连符合已冻结行为，LIFE-001、LIFE-002 使用目标断言和预期失败记录缺陷；
- 单人/双人独立 Browser Context fixture、稳定 Page Object、无冲突昵称和失败路径连接清理；
- 仅由 E2E 服务进程启用的 `E2E_FAST` Profile，正式业务逻辑和普通开发/生产配置不受影响；
- 页面异常、关键控制台错误、请求失败和 Socket.IO 断连诊断，以及失败截图、Trace、HTML 报告和附件；
- `@stable`、`@lifecycle-issue` 和未分类清单的独立收集与执行命令。

Phase 2 的直接治理输入：

- LIFE-001：算术或默写参赛者在比赛进行中离开后，剩余玩家仍被旧对局阻塞；
- LIFE-002：非参赛者可以发送认输事件并清理其他玩家的 RPS 对局；
- 当前离开/断线清理逻辑按游戏类型分支，房间状态、对局状态和机器人调度尚未形成统一生命周期入口；
- 游戏事件的房间成员、对局参与者和终局重赛权限需要统一检查；
- 重复操作、过期输入、终局参赛者离开和旧清理与新开局交错缺少系统化幂等基线。

## 2. Phase 1 范围

本阶段包含：

- 整理 Playwright 配置、测试装配、诊断产物和场景清理；
- 重构现有猜拳 E2E，消除跨测试状态依赖和随机结果依赖；
- 扩展 Page Object，覆盖首页、房间、三种游戏面板及比赛结果；
- 补齐房间与角色的正常交互基线；
- 补齐算术和默写的完整比赛主链路；
- 增加返回房间、认输和猜拳重赛的现状回归；
- 建立生命周期问题记录机制，为下一阶段服务端治理提供可复现输入；
- 运行 E2E 清单检查和完整 E2E，形成当前服务端行为基线。

本阶段不包含：

- 修复房间或对局生命周期缺陷；
- 改写 `roomManager`、`gameManager`、Socket.IO Handler 或机器人调度业务逻辑；
- 新增游戏模式、修改计分规则或调整正式环境游戏时长；
- 通过 mock Socket.IO、拦截游戏事件或伪造服务端响应代替真实链路；
- 将已确认的错误行为写成正式通过断言；
- 管理端 Playwright acceptance、生产 Nginx 或公网网关验收。

## 3. E2E 基线约束

- 每个可独立判断结果的场景必须在单个 Playwright `test` 内完成，不依赖其他测试先创建玩家、选择角色或推进比赛；
...
- 选择器优先使用角色、标签和稳定的 `data-testid`，避免依赖 Ant Design 内部 class、DOM 层级、动画样式或纯序号；
...
| 1d | 建立场景级 fixture 或辅助工厂，统一创建单人/双人独立 Context、进入房间、关闭 Context，并确保失败时也释放 Socket.IO 连接；昵称统一使用 `e2e-${process.pid}-${运行随机种子}-${递增序号}`（实现中额外插入玩家标签段，形如 `e2e-${pid}-${seed}-${label}-${seq}`，用于区分双人场景的 A/B 与单人 p），避免跨运行或未来多 worker 冲突 | `client/tests/e2e/fixtures/` 或等价目录 | ✅ |
...
| 猜拳 | 双人完整比赛 | 2 | 稳定基线 | 轮次、比分、胜负和赛果正确 |
| 猜拳 | 人机完整比赛 | 1 | 稳定基线 | 机器人正常响应并产生合法赛果 |
| 猜拳 | 认输、返回房间、重赛 | 2 | 稳定基线 | 双方状态收敛，可继续开始新比赛 |
| 算术 | 错误答案、正确答案、完整赛果 | 1–2 | 稳定基线 | 反馈、题目推进、比分和排名正确 |
| 默写 | 难度、发音、图片、输入与错误反馈 | 1–2 | 稳定基线 | 核心 UI 和跨浏览器设置同步正确 |
| 默写 | 完整比赛与赛果 | 1–2 | 稳定基线 | 真实判定、计分、排名和返回房间闭环 |
...
- Playwright 能收集全部稳定基线和待修复场景，测试分类清晰；
- 每个稳定场景可以独立运行，不依赖 spec 执行顺序或上一个测试遗留状态；
- 双人场景使用独立 Browser Context，测试结束后不残留真人玩家；
- 三种游戏模式均至少有一条经过真实 Socket.IO 的浏览器完整链路；
- 猜拳固定赛果不依赖机器人随机选择，算术正确答案不依赖服务端内部数据；
- 默写完整比赛在可接受时间内结束，且没有测试专用答案接口或跳过正式判定；
- 房间、角色、模式、难度、认输、返回房间和重赛具备用户可观察断言；
- 稳定基线连续完整执行至少 3 次无随机失败；
- 每次失败均能获得截图、Trace 和足够的页面错误诊断；
...
- 冻结等待中、进行中和终局状态下的生命周期及权限矩阵；
- 统一对局清理、机器人调度清理、剩余参赛者通知和房间状态广播；
- 修复算术、默写参赛者在比赛中主动离开或断线后旧对局未取消的问题；
- 修复非参赛者可以认输并清理他人猜拳对局的问题；
- 收敛挑战、游戏输入、认输和重赛的房间成员及对局参与者权限；
- 清理包含已离开参赛者的终局对局，禁止通过过期 Socket 重赛；
- 补齐重复操作、过期输入和快速连续操作的幂等基线；
- 增加不包含敏感业务数据的生命周期诊断日志；
- 使用服务端单元测试、真实 Socket.IO 集成测试和 Phase 1 E2E 完成回归。

本阶段不包含：

- 新增游戏模式、调整计分规则、题目规则或正式环境时长；
- 在答题比赛中动态移除玩家后让其余玩家继续比赛；
- 刷新恢复、断线续局、角色恢复、跨进程房间恢复或数据持久化；
- 修改现有 Socket.IO 事件名称或要求客户端迁移到新的主要 Payload 结构；
...
- 所有模式的游戏输入继续进入正式 GameMode 判定；Handler 只负责房间、参与者、状态和生命周期边界，不复制出拳或答题算法；
- 对局清理必须经过单一入口，同时清除 `room.game` 和对应机器人调度；调用方不得只清其中一项；
- 清理前先保存游戏快照和通知对象，清理后只向仍在线的真人参赛者发送取消消息；机器人不接收取消通知；
- 同一对局的重复清理必须幂等，不得重复发送取消消息、重复结算、影响后续新对局或遗留定时任务；
- 非法操作只向调用者发送 `game:error`，不得向房间广播错误，也不得改变角色、比分、题目、轮次或对局状态；
- 保持现有 `game:cancelled`、`game:error`、`game:start` 等事件兼容；取消消息继续使用 `{ message }`，本阶段不强制客户端理解新的原因码；
- 生命周期日志只记录事件、房间、游戏类型、操作者、结果及拒绝/清理原因；禁止记录 Cookie、Token、Local Storage、完整答案内容或其他敏感数据。

## Phase 2：服务端房间与对局生命周期治理

| 步骤 | 内容 | 涉及文件 | 状态 |
|------|------|----------|------|
| 2a | 冻结生命周期状态与权限矩阵：逐项定义等待中、进行中、终局状态下挑战、输入、认输、主动离开、断线和重赛的合法调用者、状态变化、通知对象和清理结果；把 LIFE-001、LIFE-002 映射到目标规则 | `step.md`, `docs/acceptance/v3.6/lifecycle-design.md`, `docs/acceptance/v3.6/lifecycle-issues.md` | ✅ |
...
本节是执行摘要；完整术语、校验顺序、错误文案、三状态操作明细、统一清理顺序及幂等约束以 `docs/acceptance/v3.6/lifecycle-design.md` 为准。

| 状态/操作 | 合法调用者 | 目标行为 | 非法调用行为 |
|-----------|------------|----------|--------------|
| 等待中发起挑战 | 当前房间内满足模式要求的玩家；请求房间必须等于 Socket 当前房间 | 使用服务端房间状态确定参赛者并创建对局 | 只向调用者返回 `game:error`，房间状态不变 |
| 进行中提交出拳/答案 | 当前对局参赛者 | 进入正式 GameMode 判定并按结果推进 | 非参赛者或过期输入被拒绝，不改变比分、轮次或题目 |
| 进行中认输 | 当前对局真人参赛者 | 清理对局和机器人调度，向其他在线真人参赛者各通知一次 | 返回“你不是本局玩家”或无进行中比赛错误，原对局不变 |
| 进行中参赛者主动离开 | 真人参赛者 | 取消整场比赛，通知剩余在线真人参赛者，再释放离开者的成员和角色状态 | — |
| 进行中参赛者断线 | 真人参赛者 | 与主动离开采用相同取消规则，再执行断线成员和角色清理 | — |
| 进行中旁观者离开/断线 | 非参赛者房间成员 | 只清理该成员及其角色，不影响对局 | — |
| 终局参赛者离开/断线 | 上一局真人参赛者 | 清理旧终局和调度，不发送“比赛中取消”通知 | 重赛不能再引用已离开的 Socket |
| 终局发起重赛 | 上一局真人参赛者，且原真人参赛者仍在当前房间；`game:rematch` 仅用于 RPS | 先完成授权，再清旧终局并创建全新对局，轮次和比分重置 | 非参赛者、成员不完整或模式不支持时只返回 `game:error`，原终局保留 |
| 重复或过期操作 | 无新的合法状态转换 | 安全拒绝或幂等忽略 | 不重复广播、不重复结算、不清除新对局 |

## Phase 2 测试分层

- **Handler 单测**：穷举房间成员、对局参与者、状态和事件权限，验证调用、清理、通知及错误对象；
...
`docs/acceptance/v3.6/phase-2-report.md` 至少包含：

1. **范围与结论**：2a–2j 完成状态、是否达到 Phase 2 完成条件、未通过项；
2. **规则实现映射**：状态/权限矩阵规则对应的 Handler、单元测试、集成测试和 E2E；
3. **问题关闭记录**：LIFE-001、LIFE-002 的原现象、修复方式、目标断言结果和 Tag 迁移；
4. **测试结果**：命令、环境、通过/失败数量、耗时和失败清单；
5. **诊断结果**：残留房间、残留真人 Socket、机器人调度、重复通知及关键生命周期日志检查；
6. **兼容性说明**：Socket.IO 事件和 Payload 是否保持兼容，以及存在但推迟处理的行为；
7. **下一阶段输入**：全量回归和版本发布需要继续验证的项目。
...
npm run test:e2e:stable --prefix client
npm run test:e2e:lifecycle --prefix client
npm run test:e2e:check:untagged --prefix client
git diff --check
```
...
- 重复或过期操作不会重复通知、重复计分、重复结算、清除新对局或遗留机器人任务；
- 生命周期日志足以定位房间、游戏类型、事件、操作者和原因，且不包含敏感数据或完整答案；
- 服务端单测、真实 Socket.IO 集成测试、客户端单测、稳定 E2E 和生命周期专项 E2E 全部达到预期结果；
- fixture 和 Socket 清理后无残留真人玩家，服务端无残留房间对局或机器人调度；
- 形成 `docs/acceptance/v3.6/phase-2-report.md`，记录问题关闭、测试结果、兼容性结论和下一阶段输入。
...
将游戏端和管理端从 Ant Design 5 升级到 Ant Design 6，将服务端从 Jest 29 升级到 Jest 30，并统一两个前端的 React/React DOM 精确版本。升级后的外部可观察业务行为应与 v3.6 保持一致。

计划目标版本以 2026-08-09 的 npm `latest` 为基线，实施安装前必须再次确认稳定标签，并通过各子包锁文件固定实际解析版本：

| 依赖 | 当前版本 | 目标版本 | 范围 |
|------|----------|----------|------|
| `antd` | `5.29.3` | `6.5.2` 或实施时最新 6.x 稳定版 | `client`, `admin-client` |
| `@ant-design/icons` | `5.6.1`；游戏端源码显式导入且锁文件已解析，但 `package.json` 未声明 | 与 antd 6 兼容的最新 6.x 稳定版 | `client`, `admin-client` 均显式声明直接依赖 |
| `react` / `react-dom` | 两端均声明 `^19.2.7`，锁文件分别解析为 19.2.7 和 19.2.8 | 两端 `package.json` 与锁文件统一到 19.2.8，或实施时最新 19.2.x 精确版本 | `client`, `admin-client` |
| `jest` | `29.7.0` | `30.4.2` 或实施时最新 30.x 稳定版 | `server` |
| `@types/jest` | 30.x，与运行器主版本错位 | 与实际 Jest 30 对齐 | `server` |

本版本不包含 Vitest 迁移、服务端 ESM 迁移、UI 重设计、业务功能、协议变更、路由变更或认证调整。前端升级和服务端测试升级必须形成独立可验证变更，不得混成一次不可拆分的大改。

## 实施环境说明

- v3.7 验证统一记录在 `docs/acceptance/v3.7/verification.md`；
- Supertest/Koa 接口测试会启动临时本地监听，必须在允许监听本地端口的环境执行；
- 受限沙箱中的 `listen EPERM` 属于实施环境限制，不得记为 Jest 30 或服务端行为回归；
- 视觉验收沿用既有普通电脑大分辨率口径：1366×768、1440×900、1920×1080；本版本不验收手机、平板或窄屏适配。
...
| 步骤 | 内容 | 验收要求 | 状态 |
|------|------|----------|------|
| 3a | 运行游戏端单元测试 | 现有 87 项基线全部通过；新增兼容测试按实际计入 | ✅ |
| 3b | 运行管理端单元测试 | 现有 59 项基线全部通过；新增兼容测试按实际计入 | ✅ |
| 3c | 分别及组合构建两个前端并验证产物隔离 | 构建成功；无新增阻断警告；目录不互相覆盖 | ✅ |
| 3d | 检查浏览器控制台 | 无 React 19、antd、icons、废弃 API、渲染或 Hydration 兼容错误 | ✅ |
| 3e | 运行游戏端稳定 E2E 与生命周期专项 | 三种模式、房间、认输、离开、赛果、返回和重赛行为保持一致；稳定 E2E 不少于 12 项，生命周期专项不少于 1 项 | ✅ |
| 3f | 运行管理端验收 | 登录、导航、状态、词库保存、图片同步、换图和 TTS 正常 | ✅ |
| 3g | 对照 Phase 0 截图，在 1366×768、1440×900、1920×1080 三种桌面分辨率进行视觉检查 | 无阻断性布局、Modal、滚动、图标、间距和控件状态回归；不新增手机、平板或窄屏验收 | ✅ |

### Phase 3 结果（2026-08-11）

完整记录见 [`docs/acceptance/v3.7/verification.md`](docs/acceptance/v3.7/verification.md) Phase 3 章节。

- **3a/3b/3c**：已在 Phase 2f 完成（client 87 / admin 59 单测全过、废弃警告清零、构建与隔离检查通过）。
- **3d**：三层验证--E2E fixtures 捕获 pageerror（12+1 全过=无 JS 错误）、domSnapshot 确认渲染正常、单测废弃警告清零；无 React 19/antd/icons/Hydration 兼容错误。
- **3e**：稳定 E2E 12 passed（2.2m）、生命周期专项 1 passed（50.1s），与 Phase 0 基线一致，三种模式主链路无回归。
- **3f**：browser-use 验证管理端登录/导航/词库（.ant-switch=36/.ant-progress=1）/图片同步状态/TTS 播放，全部正常。
- **3g**：CSS 类全部保留（`.ant-image-img`/`.ant-typography`/`.ant-menu`/`.ant-layout`/`.ant-switch`/`.ant-progress`/`.ant-tag`）；3 处脆弱 CSS 选择器确认无需修复（2e 结论）；v6 截图 5 张存于 `v6-screenshots/`（1440×900）供与 Phase 0 `baseline-screenshots/` 对比；三种分辨率全量视觉对照建议用户对照截图最终确认。

antd 6 升级验证完成，无功能回归、无控制台错误、CSS 无破坏。可进入 Phase 4（升级 Jest 30）。
...
| 步骤 | 内容 | 验收要求 | 状态 |
|------|------|----------|------|
| 5a | 运行根项目全量单元测试 | 服务端、游戏端、管理端全部通过 | ✅ |
| 5b | 运行 Socket.IO 真实集成测试 | 三种游戏主链路、生命周期和权限断言全部通过 | ✅ |
| 5c | 运行两个前端生产构建与构建隔离检查 | 产物路径和公网 Base 保持正确 | ✅ |
| 5d | 运行游戏端 E2E、管理端验收检查和网关离线检查 | 现有发布门禁全部达到 v3.6 基线 | ✅ |
| 5e | 检查最终依赖树、弃用警告、重复依赖、安全审计和锁文件可复现性 | 无升级引入的阻断问题 | ✅ |
| 5f | 将根项目、`client`、`admin-client`、`server` 版本统一更新为 `3.7.0` | 四个版本一致 | ✅ |
| 5g | 更新 README、AGENTS 命令/架构事实、路线图、实施步骤和 v3.7 发布说明 | 文档与实际实现一致 | ✅ |
| 5h | 在本机预发布执行 `/family-war/`、`/admin/`、标准 API、图片、Socket.IO polling/WebSocket 全链路验收 | 无协议、路由、认证或游戏行为回归 | ✅ |
| 5i | 完成 `git diff --check`、变更审查和 v3.7 验证报告；不自动暂存或提交 | 无格式错误，报告可复核 | ✅ |

### Phase 5 结果（2026-08-11）

完整记录见 [`docs/acceptance/v3.7/verification.md`](docs/acceptance/v3.7/verification.md) Phase 5 章节。

- **5a**：根项目全量单测--server 342 / client 87 / admin 59 全过。
- **5b**：Socket.IO 集成 109/0 通过。
- **5c**：两端生产构建成功，构建隔离检查通过。
- **5d**：E2E 清单 13 项、管理端验收离线 7 步骤、网关离线检查均通过。
- **5e**：依赖树无重复 React/antd、无 peer 冲突；npm audit 因 registry 镜像不支持 audit API 不可用（环境限制，非升级问题）；锁文件可复现性由安装成功验证。
- **5f**：根项目、client、admin-client、server 版本统一 `3.7.0`。
- **5g**：AGENTS.md（Antd v5->v6、Jest v29->v30）、README.md（Jest v30）、road-map.md（v3.7 已完成 + 发布结果）已更新。
- **5h**：dev 环境全链路验收--API health 200、/family-war/ 200、/admin/ 200、/api/images/classroom.jpg 200、Socket.IO polling 握手 200。
- **5i**：`git diff --check` 通过；v3.7 验证报告齐全（`docs/acceptance/v3.7/verification.md`）；不自动暂存或提交。

v3.7.0 全量回归与发布准备完成，发布门禁全部满足。

## v3.7.0 发布门禁

- Ant Design 与 icons 均为相互兼容的 6.x 稳定版本，React/React DOM 在两个前端完全一致；
- Jest 与 `@types/jest` 主版本一致，服务端全部测试在 Jest 30 下执行；
- 依赖树不存在重复 React、重复 antd、invalid peer 或意外保留的 v5 React 19 补丁；
- 游戏端、管理端和服务端单元测试达到或超过 v3.6 基线；
- Socket.IO 集成、游戏端稳定 E2E、生命周期专项、管理验收和构建验证全部通过；
- 关键页面在 1366×768、1440×900、1920×1080 三种桌面分辨率没有阻断性视觉变化，浏览器控制台没有新增兼容错误；手机、平板和窄屏不属于本版本发布门禁；
- 生产服务端源码、Socket.IO 协议、公网路由、管理员认证和游戏规则没有越界修改；
- 根项目和三个子包版本均为 `3.7.0`，升级说明、验证报告和发布说明齐全。

## 建议实施顺序与回退边界

1. 先完成并验证 Ant Design 6；出现 UI 回归时只在前端升级范围内处理或回退。
2. 前端门禁通过后再升级 Jest 30；出现测试差异时保持生产服务端代码不变，优先修复测试兼容。
3. 两个阶段均通过后再更新版本号和发布文档。
4. Ant Design、Jest 和发布准备分别保持独立变更边界，便于审查、定位和回退。
