...
|-----------|------------|----------|--------------|
| 等待中发起挑战 | 当前房间内满足模式要求的玩家 | 使用服务端房间状态确定参赛者并创建对局 | 只向调用者返回 `game:error`，房间状态不变 |
| 进行中提交出拳/答案 | 当前对局参赛者 | 进入正式 GameMode 判定并按结果推进 | 非参赛者或过期输入被拒绝，不改变比分、轮次或题目 |
| 进行中认输 | 当前对局参赛者 | 清理对局和机器人调度，通知其他在线真人参赛者 | 返回“你不是本局玩家”或无进行中比赛错误，原对局不变 |
...
| 终局发起重赛 | 上一局参赛者，且原真人参赛者仍在当前房间 | 复用正式开局逻辑创建全新对局，轮次和比分重置 | 非参赛者或成员不完整时只返回 `game:error` |
