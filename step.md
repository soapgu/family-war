...
> 当前仅细化 Phase 1“游戏端 E2E 基线”。服务端房间与对局生命周期治理、全量回归和版本发布将在后续逐阶段补充。

## 1. 当前基础

游戏端已经使用 Playwright 和 Chromium 建立真实浏览器 E2E，默认访问本地 Vite 页面并通过开发代理连接真实 Koa、Socket.IO 服务。现有覆盖包括：

- 石头剪刀布双人完整比赛；
- 石头剪刀布人机完整比赛；
- 首页、房间和猜拳游戏的基础 Page Object；
- 失败截图、Trace 和 HTML 报告；
- 单 worker 串行执行。

当前主要缺口：

- 算术和默写没有浏览器级完整比赛覆盖；
- 房间模式切换、角色同步、离开、认输、重赛等行为缺少独立场景；
- 现有双人猜拳用例通过 `beforeAll` 跨多个测试共享浏览器和服务端状态，前序失败会导致后续级联失败；
- 人机猜拳依赖随机出拳及最多循环次数，只证明最终出现赛果，没有验证关键过程和胜负语义；
- 默认比赛分数、题目时限和机器人延迟较长，不利于快速、稳定地重复执行算术和默写完整比赛；
- 所有场景共用 `default` 房间，测试结束时必须可靠退出或关闭连接，否则后续场景可能受到残留玩家和比赛状态影响；
- 当前缺少对控制台错误、未处理页面异常和关键 Socket.IO 断连的统一诊断。
