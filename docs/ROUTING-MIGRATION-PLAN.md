...
`family-war` 后端未来需要增加平台普通用户会话验证。v3.5 已将现有单一管理员密码认证
拆到 `/api/admin-auth/login|me|logout`，它仍是管理员专用会话，不等同于未来的平台
普通用户认证。两类身份应明确区分：

- 平台会话：识别微信账户、家庭和当前活动档案；
- 管理员会话：使用 `admin_session` 和管理员专用 JWT 声明进入后台；
- 应用权限：判断当前档案或管理员是否可以进入应用、查看状态或执行管理操作。

未来普通用户 Token 不直接复用管理员 Cookie、声明空间和验证中间件；是否使用独立
Secret 或独立签发服务，应在微信与平台用户身份版本中单独设计。
...
2. 旧 HTTP API 不重定向到标准入口，避免隐藏仍未升级的调用方；
3. Socket.IO 旧路径不采用普通页面式重定向；
4. 删除旧 Nginx location；
5. 更新部署文档和运维检查项。

v3.5 已完成两个旧 location 的分步删除、`removed` 网关验收和恢复演练。回滚片段继续
保留在 `deploy/nginx/family-war-legacy-locations.conf`，但正常配置不再启用。
...
- 管理登录、当前管理员和退出使用 `/api/admin-auth/*`，family-war 状态及单词图片管理使用 `/api/family-war/*`；
